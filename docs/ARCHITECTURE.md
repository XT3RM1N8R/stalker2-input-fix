...
When intercepting Windows Kernel file I/O (`CreateFile` and `ReadFile`) or `RAWINPUT` messages, two strict rules must be followed:
...
2. **Handle Lifecycle (Avoid Reuse Leaks):** Because Windows aggressively recycles handle IDs, tracking `HANDLE` pointers requires absolute lifecycle management. You must hook `CloseHandle` to instantly purge dropped kernel handles from internal watch-lists. For RawInput device handles, you must intercept `WM_INPUT_DEVICE_CHANGE` (`0x00FE`) and check for `GIDC_REMOVAL`, dropping the `lParam` handle from your cache. Failing to do this causes the OS to reuse the handle ID for new devices (like an Xbox controller or a media stream), causing the proxy to incorrectly spoof or corrupt their telemetry.
