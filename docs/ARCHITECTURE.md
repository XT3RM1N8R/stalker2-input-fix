...
### The Perfect Hardware Spoofed Packet
...
2. **DualSense Buttons:** In the 64+ byte DualSense payload, the triggers occupy bytes 5/6, and the buttons occupy byte 8. A `0` in byte 8 forces the D-Pad to the "UP" position (`0x00` = Up, `0x08` = Neutral). 
3. **DualShock 4 Buttons:** In the DS4 payload, the layout is inverted. The buttons/D-pad occupy byte 5, and the triggers occupy bytes 8/9. Sending a DualSense packet to a DS4 parsing function will result in permanently stuck D-pads.
4. **Gyroscope Gravity:** The accelerometer natively reports 1G of gravity (`8192`) on the Z-Axis (`buf[27] = 0x20` for DualSense, `buf[24] = 0x20` for DS4). Setting this to `0` puts the controller in "0G Freefall", which causes Unreal Engine's rotation math to divide by zero and produce `NaN`, instantly snapping the player's virtual mouse cursor to the top-left pixel of the monitor.
5. **Touchpad Override:** The Touchpad MSB must be explicitly set (`0x80`) to communicate "Not Touching" (Byte 33 for DualSense, Byte 36 for DualShock 4). Furthermore, the X/Y coordinates must be mathematically centered (`960x540` for DualSense, `960x471` for DS4) to protect against buggy game engine parsers that ignore the Touch Flag.
...
### Hardware Identification and the O(1) Lock-Free Tracking Table
When intercepting Windows Kernel file I/O (`CreateFile` and `ReadFile`), strict filtering and lifecycle management are essential to prevent cross-contamination with media files or other controllers:
1. **The Vendor / API Split:** 
   - **Xbox Controllers:** Xbox devices interface via the native XInput or GameInput APIs. They are supported natively by Stalker 2 without mapping issues. However, they *do* cause input bleeding while the Steam Overlay is open. The proxy safely handles Xbox controllers by intercepting `XInputGetState` and explicitly zeroing out the `XINPUT_STATE` struct. Because XInput uses signed integers where `0` is center, `memset(0)` correctly neutralizes the axes. They bypass the `ReadFile` layer entirely.
   - **Sony Controllers:** Sony gamepads operate as Generic HID devices via USB `ReadFile`. Because Stalker 2 parses them natively over USB while players simultaneously emulate Xbox inputs via Steam/DS4Windows, "Double Input" bugs occur. The proxy actively intercepts `ReadFile` for these exact Product IDs to inject mathematically neutralized packets (`0x80` for axes). Generic gamepads are purposefully ignored at the `ReadFile` layer because synthesizing an arbitrary neutral state (via `memset(0)`) risks violently throwing unmapped axes to their minimum values (e.g., spinning the camera top-left).
2. **The O(1) Lock-Free Architecture:** 
   Because `ReadFile` is intercepted globally across all OS handle reads, lookups must execute instantly. Windows assigns handle IDs as multiples of 4, up to a theoretical max of 16,777,216 handles per process. The proxy statically allocates a 16MB direct-mapped array (`g_HandleTable`). When `ReadFile` is called, the proxy calculates the array index (`hFile / 4`) and checks the `std::atomic` tracking byte in a single lock-free CPU instruction, ensuring zero lock-contention, zero input latency, and zero audio/video stutter.
3. **Handle Lifecycle Mirroring:**
   Because Windows aggressively recycles handle IDs, tracking `HANDLE` values creates Use-After-Free vulnerabilities (e.g., the OS recycles a closed controller handle for a `.bk2` video file, and the proxy accidentally corrupts the video stream). To prevent this, the proxy hooks `ntdll.dll!NtClose`. Because `NtClose` is the absolute lowest-level wrapper for handle destruction, the proxy perfectly catches every closed handle—even if the engine bypasses `kernel32`—and instantaneously erases the tracking byte. This perfectly mirrors the OS kernel state.
